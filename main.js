...
var $author$project$TransferPage$Model = F8(
	function (amount, fromAccount, toAccount, date, info, notes, pageState, transferType) {
		return {amount: amount, date: date, fromAccount: fromAccount, info: info, notes: notes, pageState: pageState, toAccount: toAccount, transferType: transferType};
	});
var $author$project$TransferPage$Transfer = {$: 'Transfer'};
var $author$project$TransferPage$init = A8($author$project$TransferPage$Model, '', '', '', '', '', '', $author$project$TransferPage$Loading, $author$project$TransferPage$Transfer);
...
var $author$project$Auth$clientId = '34596233405-fgqv4u6utob1c9hkf596g42r6dhdr20d.apps.googleusercontent.com';
...
var $author$project$Page$errToString = function (e) {
	switch (e.$) {
		...
		case 'BadStatus':
			var _int = e.a;
			return 'Server responded with a bad status code: ' + ($elm$core$String$fromInt(_int) + '. Try quitting the app completely and re-opening it.');
		case 'BadBody':
			var s = e.a;
			return s;
		case 'NetworkError':
			return 'Network error! Unable to make a request.';
		default:
			return 'Network timeout! Took too long and there is no response yet.';
	}
};
...
var $author$project$Auth$authUrl = 'https://accounts.google.com/o/oauth2/token';
				...
				if (res.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						...
						$author$project$Capacitor$showAlert(
							{
								message: $author$project$Page$errToString(e),
...
var $author$project$TransferPage$Investment = {$: 'Investment'};
var $author$project$TransferPage$Saving = {$: 'Saving'};
var $author$project$TransferPage$SaveTransferDone = function (a) {
	return {$: 'SaveTransferDone', a: a};
};
var $author$project$TransferPage$transferTypeToString = function (ttype) {
	if (ttype.$ === 'Transfer') {
		return 'transfer';
	} else {
		return 'investment';
...
var $author$project$TransferPage$update = F3(
	function (msg, model, globals) {
		switch (msg.$) {
			...
			default:
				var toggle = function (ttype) {
					if (ttype.$ === 'Transfer') {
						return $author$project$TransferPage$Investment;
					} else {
						return $author$project$TransferPage$Transfer;
					}
				};
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							transferType: toggle(model.transferType)
						}),
					$elm$core$Platform$Cmd$none);
		}
	});
...
var $author$project$TransferPage$ToggleTransferType = function (a) {
	return {$: 'ToggleTransferType', a: a};
};
						...
						A2(
							...
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											$elm$html$Html$Events$onCheck($author$project$TransferPage$ToggleTransferType),
											$elm$html$Html$Attributes$checked(
											_Utils_eq(model.transferType, $author$project$TransferPage$Investment)),
											$elm$html$Html$Attributes$type_('checkbox'),
											$elm$html$Html$Attributes$id('transfer-type')
										]),
									_List_Nil),
									A2(
									$elm$html$Html$label,
									_List_fromArray(
										[
											$elm$html$Html$Attributes$for('transfer-type')
										]),
									_List_fromArray(
										[
											$elm$html$Html$text('Yes, it\'s an investment')
										]))
								]))),
						A2(
						$author$project$Page$formElement,
